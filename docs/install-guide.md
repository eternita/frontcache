...
`guard-rules.conf`, `fc-l1-ehcache-config.xml`, `resilience.properties`, and `conf/frontcache.id`
...
1. Terminate TLS upstream (nginx, ALB) on 80/443 and proxy to `http://127.0.0.1:9080`. Forward
   ...
   and the management-port check correct behind a proxy. Do **not** buffer `/fc-dashboard.stream`
   (or its permanent legacy alias `/hystrix.stream`); it is a Server-Sent-Events stream. Set
   `front-cache.http-port` / `front-cache.https-port` to the ports **clients** see (80/443), not
   9080, or redirects will send users to `:9080`.
...
**Coming from 2.6.x, two files carried across need attention.** `hystrix.properties` was renamed to
`resilience.properties`; the old name is still read for this one release, with a warning, and every
property key inside it keeps working indefinitely — so rename the file when convenient, and do not
leave both present (the new name wins, which would quietly revert your tuning to the shipped
defaults). `fc-logback.xml` needs a hand edit: the fallback logger was renamed with the package, and
a carried-over file leaves `logs/fallback.log` silently empty. The node warns once at startup with
the exact edit.
...
circuit-breaker fallbacks). The [log-analytics example](../examples/log-analytics) indexes all four into
